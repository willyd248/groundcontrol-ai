# ANTICIPATION_DESIGN_v2.md — Anticipation Upgrade
...
1. **FIX 1 — OBS_DIM arithmetic consistency.** v1 had the breakdown table showing vehicle slots at old dim (4x4=16) totaling 333, then separately adding 4 for the `is_reserved` feature to get 337. The table, intro sentence, and total now all consistently reflect the updated vehicle dims (4x5=20) and total 337. Full arithmetic walkthrough added for auditability.

2. **FIX 2 — Reservation conversion ordering rule.** Added subsection "Reservation conversion ordering" in Section 3 specifying that when a pending task is created, the dispatcher MUST first check for a matching reservation before any auto-assign or agent-query logic. Added `test_reservation_conversion_priority` test case in Section 6.

3. **FIX 3 — Decision trigger interaction.** Added subsection "Decision trigger interaction" in Section 3 specifying that reservation conversion is automatic and does NOT trigger an agent query. Event B refined to exclude auto-converting tasks. Anticipated task visibility does NOT add a new query trigger — agent sees anticipated tasks at existing query events. Event D from v1 Open Question 7 is dropped.

4. **FIX 4 — Third kill criterion.** Added kill criterion #5 to Section 7: if at 500k reservation rate >5% BUT hard battery mean delta worse than v5 baseline of -10.4 min, stop and debug — agent is using reservations counterproductively.

5. **FIX 5 — Network architecture.** Changed Section 8 Q8 recommendation from "start at [128,128]" to "start at default [64,64], bump to [128,128] only as documented intervention if 200k explained_variance < 0.80 or reward flat by 100k." Preserves one-variable-at-a-time experimentation.

6. **FIX 6 — Seed 40 B777 verification.** Verified via `generate_schedule(seed=40, density='tight')`: seed 40 has **4 B777s** (RND01, RND06, RND08 in Wave 1; RND10 in Wave 2). Updated from "3x B777" to "3x B777 in Wave 1 (4 total)" with full aircraft mix documented.
...
### Reservation conversion ordering

**This ordering is a hard requirement for implementation, not a suggestion.**

When `_create_service_tasks()` creates a new pending task during `dispatcher.tick()`, the following sequence MUST execute in this exact order:

1. **Check for matching reservation.** For the newly created task `(flight_id, service_type)`, scan all vehicles for one where `vehicle.reserved_for` matches this `(flight_id, service_type)` pair.

2. **If match found: convert reservation immediately.** Assign the reserved vehicle to the new pending task. Set `vehicle.state = VehicleState.EN_ROUTE`, compute path to gate, clear `vehicle.reserved_for` and `vehicle.reserved_until`, move task from `pending_tasks` to `active_tasks`. This happens within the same tick, before control returns to the env.

3. **If no match: leave task in pending_tasks.** Normal flow — the task sits in `pending_tasks` and will be visible to the agent at the next decision point.

4. **Only after all reservation conversions for this tick are resolved** does the event-based trigger logic (Event A/B/C) evaluate whether to query the agent.

**Why this ordering matters:** Without it, a race condition exists where the agent is queried about a pending task that has a reservation, leading to a double-assignment (agent assigns one vehicle, auto-conversion assigns another). The ordering guarantees that by the time the agent sees any pending task, no reservation exists for it.

**Implementation note:** `reserved_for` must store enough information to match: `(flight_id, service_type)` tuple, not just flight_id, because a single flight has multiple service types and the agent may reserve different vehicles for different services on the same flight.

### Decision trigger interaction

The anticipation upgrade interacts with the event-based trigger system (DECISION_TRIGGER.md) as follows:

**Reservation conversion is automatic and silent.** When a reserved vehicle is auto-assigned to a materializing task (per the ordering rule above), this does NOT trigger an agent query. The conversion is a deterministic consequence of the agent's earlier reservation decision — querying again would be redundant. From the trigger system's perspective, the task was never "pending" (it was converted in the same tick it was created).

**Event B refined.** Event B currently fires when "new task(s) entered pending_tasks this tick AND a compatible free vehicle exists." With anticipation, this is refined to: "new task(s) entered pending_tasks this tick AND were NOT consumed by reservation conversion AND a compatible free vehicle exists." Tasks that are immediately converted by a reservation do not count toward `new_tasks_arrived` in the event detector, because they never enter the actionable pending queue.

**Anticipated tasks do NOT add a new query trigger.** v1 proposed "Event D: new anticipated task enters the queue AND a free compatible vehicle exists." This is dropped in v2. Instead, the agent sees anticipated tasks in observation at whatever query event next fires (Event A, B, or C). Rationale:
- Anticipated tasks change every tick as `time_until_actionable` decreases — triggering on every new anticipated task would re-introduce high query frequency
- The agent can only reserve at decision points; it doesn't need a dedicated trigger for reservation opportunities
- Keeping the trigger set to {A, B, C} preserves the validated query-count properties from the decision trigger fix (~40 queries/episode)
- If a reservation opportunity is time-sensitive, it will naturally coincide with an Event A or B (vehicle freed or new real task) within a few seconds
...
Features per anticipated task: 1 + 3 + 3 + 1 + 1 = **9**
Total for 8 slots: 8 x 9 = **72**
...
Each vehicle slot gains one feature to encode reservation status:

| Feature | Encoding | Range |
|---------|----------|-------|
| `is_reserved` | 1 if vehicle has an active reservation | {0, 1} |

This changes VEH_FEATURES from 4 to **5**.

### New observation dimension — full arithmetic

```
OBS_DIM = 1                            # sim_time_norm
        + MAX_AIRCRAFT  * AC_FEATURES  # 20 * 8  = 160  (unchanged)
        + MAX_VEHICLES  * VEH_FEATURES # 4  * 5  = 20   (was 4*4=16; +4 from is_reserved)
        + MAX_TASKS     * TASK_FEATURES# 16 * 5  = 80   (unchanged)
        + 1                            # n_pending_norm  (unchanged)
        + MAX_ANTICIPATED * ANT_FEATURES # 8 * 9 = 72   (NEW)
...
| Component | Features/slot | Slots | Current dims | New dims |
|-----------|--------------|-------|-------------|----------|
| sim_time | 1 | 1 | 1 | 1 |
| Aircraft slots | 8 | 20 | 160 | 160 |
| Vehicle slots | 4 -> **5** | 4 | 16 | **20** |
| Pending task slots | 5 | 16 | 80 | 80 |
| n_pending_norm | 1 | 1 | 1 | 1 |
| Anticipated task slots | **9** | **8** | 0 | **72** |
| Global anticipation features | — | — | 0 | **3** |
| **Total** | | | **258** | **337** |

**New OBS_DIM = 337** (was 258, +79 dims / +30.6% increase).
...
### Step 4: Reservation state on vehicles + conversion ordering (complexity: medium-high, ~3 hours)

- Add `reserved_for: Optional[tuple[str, str]]` (flight_id, service_type) and `reserved_until: float` to `Vehicle`
...
- **Implement reservation conversion ordering** (Section 3): in `_create_service_tasks()`, check for matching reservation BEFORE appending to `pending_tasks`. If match found, convert immediately to active task.
- Update Event B detection to exclude tasks consumed by reservation conversion

Files: `sim/entities.py`, `sim/dispatcher.py`, `env/airport_env.py`
...
- `test_reservation_conversion_priority`: create scenario with two compatible vehicles — one reserved for an upcoming flight's fuel task, one free. Advance sim until flight reaches gate and fuel task is created. Assert: reserved vehicle gets the assignment, not the free one. Assert: task never appears in `pending_tasks` (converted in same tick). Assert: free vehicle remains available for other assignments.
...
| Seed 40 delta | < +15 min | +27.6 min (estimated from FAILURE_DIAGNOSTIC.md pattern) | Seed 40 is worst-case B777 contention (3x B777 in Wave 1, 4 total); should benefit most from anticipation |
| Conflict rate | 0% | 0% | No regression on safety |
| Reservation expiry rate | < 50% of reservations | N/A | Agent not over-reserving |

### Seed 40 reference data (verified)

```
generate_schedule(seed=40, density='tight')
17 flights (15 turn + 2 dep-only), 4 B777s total

Wave 1 (t=125-535s, 8 flights):
  RND01 B777   t=125s
  RND02 A320   t=210s
  RND03 CRJ900 t=245s
  RND05 CRJ900 t=359s
  RND06 B777   t=381s
  RND07 B737   t=399s
  RND08 B777   t=506s    ← 3rd B777 in wave 1
  RND09 A320   t=535s

Wave 2 (t=3798-4072s, 7 flights):
  RND10 B777   t=3798s   ← 4th B777
  RND11 A320   t=3912s
  RND12 A320   t=3920s
  RND13 B737   t=3921s
  RND14 A320   t=3935s
  RND15 B737   t=3993s
  RND16 CRJ900 t=4072s

Dep-only: RND04, RND17
```
...
**Stop training and reassess if any of these hold:**

1. **Reservation rate near 0% at 500k** — agent is not using the new capability at all. Indicates obs encoding is broken, mask logic prevents reservation exploration, or reward signal is too weak.
...
5. **Reservation rate > 5% at 500k BUT hard battery mean delta worse than -10.4 min (v5 baseline)** — agent is actively using reservations but making things worse. This is not a learning speed issue; it indicates a reward or mask bug causing counterproductive reservations. Stop and debug before continuing to 1M. Likely causes: reservation conversion ordering bug (double-assignment), expiry penalty too weak (reserving ties up vehicles with no cost), or mask allowing reservations when immediate assignment would be better.
...
7. **~~Event D trigger for anticipated tasks?~~** Resolved in v2: **No Event D.** Anticipated tasks do not add a new query trigger. The agent sees anticipated tasks in observation at the next existing query event (A, B, or C). See "Decision trigger interaction" subsection in Section 3 for rationale.

8. **OBS_DIM 337 vs 258 — should we also increase network width?** MlpPolicy default is [64, 64]. With 337 inputs (vs 258), the first hidden layer compresses 5.3:1. Recommendation: **start at default [64, 64].** Bump to [128, 128] only as a documented intervention if: (a) 200k smoke shows `explained_variance` below 0.80, OR (b) reward is flat by 100k with no learning signal. This preserves one-variable-at-a-time experimentation — the obs/action space change is already a large variable; changing network width simultaneously confounds the signal. If we need to bump, it becomes a separate tracked experiment.
